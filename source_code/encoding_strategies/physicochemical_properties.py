...
class physicochemical_encoder(object):
    ...
    def encoding_dataset(self):
        ...
        data_encoding = Parallel(n_jobs=6, require='sharedmem')(delayed(self.__encoding_sequence)(self.dataset[self.name_column_seq][i], self.dataset[self.name_column_id][i]) for i in range(len(self.dataset)))
